...
local LocationJSONFilename = ModPath..'hud3_rooms'
		...
		if category == 0 then -- Unit
			...
			if cCarry then
				...
				elseif cHealth == 0 then
                    ...
                    local isSpecial = tweak_data.character[ unitTweak ]
                    isSpecial = isSpecial and isSpecial.priority_shout
                    for i = 1, 4 do
                        if Poco:StatStatic(i,'minion')==unit
                                or Poco:StatStatic(i,'minion2')==unit
                        then
                            isMinion = true
                            break
                        end
                    ...
                    elseif isCiv then
                        if O:get('float','showTargetsCivs') then
                            showTarget = true
                        else
                            prog = 0
                        end
                    elseif isSpecial then
                        if O:get('float','showTargetsSpecials') then
                            showTarget = true
                        else
                            prog = 0
                        end
                    elseif unit:base() and unit:base().sentry_gun then
                        if O:get('float','showTargetsOther') then
                            showTarget = true
                        else
                            prog = 0
                        end
                    elseif isTeamAi or (charDmg and charDmg.damage_tase) then
                        if isTeamAi then
                            if O:get('float','showTargetsTeamAi') then
                                showTarget = true
                            else
                                prog = 0
                            end
                    ...
                    elseif O:get('float','showTargetsOther') then
                        showTarget = true
                    else
                        prog = 0
                    end
		...
		if category == 1 then -- Drill
			...
			elseif tGUI then
				...
				if unit then
					...
					--Add '+' if the drill will auto-restart
					--name = self:get_name(name)..((unit:base() and unit:base()._autorepair) and '+' or '')
				end
				leftT = tGUI._time_left
				prog = 1-tGUI._time_left/tGUI._timer
				
				--time left till the next drill break
				--local jamTimer = tGUI._current_jam_timer and (' ('..self.owner:_time(tGUI._current_jam_timer*math.max(tGUI._timer_multiplier or 1, 0.01))..')') or ''
			...
			elseif dGUI then
				...
				name = 'digitalgui'
				name = self:get_name(name, 'Timelock')
				leftT = dGUI._timer
...
function PocoLocation:load()
	...
	local f,err = io.open(LocationJSONFilename..lang..LocationJSONFileExt, 'r')
	local result = false
	if f then
		local t = f:read('*all')
		...
		f,err = io.open(LocationJSONFilename..LocationJSONFileExt, 'r')
		local t = f:read('*all')
		local o = JSON:decode(t)
		if type(o) == 'table' then
			self.rooms = o
...
function Localizer:get(key,context)
	local val = PocoLocale._defaultLocaleData[key] or self.data[key] or self._data[key] or PocoHud3Class.Icon[key:gsub('_','')]
