...
PocoLocale = nil
Poco._req ('poco/Hud3_LocaleIngame'..O('root','languageIngame')..'.lua')
if not PocoLocale then
	Poco._req ('poco/Hud3_LocaleIngame.lua')
...
--	Poco:LoadOptions(self:name(1),O)
	...
	od = Global.game_settings.difficulty and Global.game_settings.difficulty == "sm_wish" or false
...
function TPocoHud3:AddDmgPop(sender,hitPos,unit,offset,damage,death,head,dmgType, isCrit)
	...
	local texts = { }
    if isCrit then
        texts[#texts+1] = {'',cl.Red}
    end
	if rDamage>0 then
		texts[#texts+1] = {_.f(rDamage*10), isCrit and cl.Yellow or color}
	end
	if head then
		texts[#texts+1] = {'!',color:with_red(1)}
	end
	if death or isCrit then
		texts[#texts+1] = {'',isCrit and cl.Red or isSpecial and cl.Yellow or color}
	end
...
function TPocoHud3:_updatePlayers(t)
	...
	for i = 1,4 do
		...
		local onlyMe = O:get('playerBottom','onlyMe') and not self.verbose
						...
						if bPnl and not (not isMe and bPnl == managers.hud._teammate_panels[4]) then
							...
							if peer and alive(peer:unit()) then
								if btmO.showRank then
									local rank = ''
									local lvl = ''
									if not isMe and O:get('playerBottom','rankToPlaytime') and playtime > 0 then
										rank = playtime..PocoLocale._hours..' '
										lvl = ''
									else
										rank = isMe and managers.experience:current_rank() or (peer and peer:rank())
										rank = rank and (rank > 0) and (self:_romanic_number(rank)..'-') or ''
		...
		if not self.custom_hud_enabled and pnl and (nData or isMe) and not self.dead then
			...
			local lives =	isMe and managers.player:upgrade_value( 'player', 'additional_lives', 0) or ((O:get('chat','assumeNineLives') and od) and 1 or 0)
            ...
            if _show('Head') then
				txts[#txts+1]={' '..Icon.Skull..head,cl.Cyan:with_alpha(0.7)}
			end
			...
			if _show('Downs') then
				local maxDownCount = od and 1 or 3
				txts[#txts+1]={' '..Icon.Ghost..downs..((lives>0 and isMe) and '/'..(maxDownCount+1) or ''),downs<maxDownCount+(lives>0 and 1 or 0) and clGood or Color.red}
			end
			if _show('Incapacitations') then
				txts[#txts+1]={'|', cl.Gray}
				txts[#txts+1]={''..downsIncap, cl.Gray:with_alpha(0.8)}
			end
			
			if not isMe and _show('Playtime') and playtime > 0 then
...
function TPocoHud3:_updateItems(t,dt)
	...
	for i = 1,4 do
		local minion = self:Stat(i,'minion')
		if minion and minion ~= 0 and alive(minion) then
			if minion:character_damage()._health > 0
				and not (minion:brain().is_current_logic
						and minion:brain():is_current_logic('trade'))
			then
			...
			elseif not self._endGameT then
				...
				local minionLocName = minion.position and self:_name(minion:position(),true) or nil
				self:Chat('minionLost',L('_msg_minionLost',{self:_name(i),self:_name(minion),(O:get('chat','includeLocation') and minionLocName) and ', '..minionLocName or ''}))
			end
		end
		local minion2 = self:Stat(i,'minion2')
		if minion2 and minion2 ~= 0 and alive(minion2)  then
			if minion2:character_damage()._health > 0 
				and not (minion2:brain().is_current_logic
						and minion2:brain():is_current_logic('trade'))
			then
				if O:get('float','showTargetsConvert') then
					self:Float(minion2,0,false,{minion = i})
				end
			elseif not self._endGameT then
				self:Stat(i,'minion2',0)
				local minionLocName = minion2.position and self:_name(minion2:position(),true) or nil
				self:Chat('minionLost',L('_msg_minionLost',{self:_name(i),self:_name(minion2),(O:get('chat','includeLocation') and minionLocName) and ', '..minionLocName or ''}))
			end
		end
	end
	-- ScanBuff
...
function TPocoHud3:_time(sec)
	...
	if sec >= 0 then
		table.insert(r,sec<60 and _.f(sec) or ('%02d'):format(math.floor(sec%60)))
	end
...
function TPocoHud3:_hook()
	...
	if inGame then
		...
		hook( PlayerIncapacitated, 'enter', function( self ,...)
			local pid = me:_pid(self._unit)
			if pid then
				me:Stat(pid,'downIncap',1,true)
			end
			return Run('enter', self,... )
		end)

		hook( UnitNetworkHandler, 'sync_player_movement_state', function(self, unit, state, down_time, unit_id_str)
			Run('sync_player_movement_state', self, unit, state, down_time, unit_id_str)
			local pid = me:_pid(unit)
			if pid and state == 'incapacitated' then
				me:Stat(pid,'downIncap',1,true)
			end
		end)
		
		--hook( HuskPlayerMovement, '_shoot_blank', function(...)
		--	Run('shot_blank', ...)
		--	managers.chat:_receive_message(1, "SYSTEM", 'shot fired', Color.green)
		--end)
		...
		hook( PlayerManager, 'use_messiah_charge', function( self)
			Run('use_messiah_charge', self)
	...
	--[[	hook( UnitNetworkHandler, 'sync_start_auto_fire_sound', function(self, shooting_unit, sender)
			Run('sync_start_auto_fire_sound', self, shooting_unit, sender )
			if self._verify_gamestate(self._gamestate_filter.any_ingame) 
				and self._verify_character_and_sender(shooting_unit, sender)
			then
				me:Stat(me:_pid(shooting_unit),'auto_fire_st',now())
			end
		end)
		
		hook( UnitNetworkHandler, 'sync_stop_auto_fire_sound', function(self, shooting_unit, sender)
			Run('sync_stop_auto_fire_sound', self, shooting_unit, sender )
			if self._verify_gamestate(self._gamestate_filter.any_ingame) 
				and self._verify_character_and_sender(shooting_unit, sender)
			then
				local supid = me:_pid(shooting_unit)
				local st = me:Stat(supid,'auto_fire_st')
				if supid > 0 and st and st > -1 and shooting_unit:inventory() then
					local weap = shooting_unit:inventory():equipped_unit()
					if weap and weap:weapon_tweak_data()
						--and weap:weapon_tweak_data().auto
					then
						local fr = weap:weapon_tweak_data().fire_rate
						local shots = fr*(now()-st)
						log('fired '..shots)
						me:Stat(me:_pid(shooting_unit),'auto_fire_st',-1)
					end
				end
			end
		end)]]

	--[[	hook( ExplosionManager, 'detect_and_give_dmg', function(self, params)
			local hit_units, splinters, results = Run('detect_and_give_dmg', self, params )

			if results
				and (results.count_cops > 0
					or results.count_gangsters > 0
					or results.count_cop_kills > 0
					or results.count_gangster_kills > 0
				    )
			then
				if params.user and alive(params.user) then
					me:Stat(me:_pid(params.user),'hit',1,true)
				end
			end
			...
			if info.col_ray or info.variant == 'poison' then
				...
				if hitPos then
					...
					if alive(realAttacker) then
						...
						elseif realAttacker:base()
								and realAttacker:base().sentry_gun
						then
							realAttacker = realAttacker:base():get_owner()
						end
					end
					
					me:AddDmgPop(realAttacker,hitPos,self._unit,0,info.damage,self._dead,head, info.variant, info.critical_hit and O:get('popup','myCrits')) --info.variant ~= 'explosion' and info.variant or 'he')
				end
			end
        ...
        hook( CopDamage, 'roll_critical_hit', function(self, attack_data)
			local critical_hit, damage = Run('roll_critical_hit',self, attack_data)
            if O:get('popup','myCrits') and critical_hit then
                local caller = debug.getinfo(2,'n')
                caller = caller and caller.name or ""
                if attack_data.is_fire_dot_damage and caller == "damage_fire" then
                    if damage%1 == 0 then
                        attack_data.critical_hit = true
                    end
            elseif #(tostring(damage%1)) <= 3 then
                    attack_data.critical_hit = true
                end
            end
		...
		local onReplenish = function(pid,noReset,fak)
			...
			local downStr = down>0 and L('_msg_replenishedDown'..(down>1 and 'Plu' or ''),{down}) or (fak and L('_msg_replenishedDownFak') or '')
		...
		local OnCriminalDowned = function(pid)
			...
			if (self:Stat(pid,'down') or 0) >= (od and (O:get('chat','assumeNineLives') and 2 or 1) or 3) then
		...
		hook( HuskPlayerMovement, '_perform_movement_action_enter_bleedout', function( self, ... )
			local pid = me:_pid(self._unit)
			if pid then
				OnCriminalDowned(pid)
			end
			--return Run('_start_bleedout', self,  ...)
			Run('_perform_movement_action_enter_bleedout', self,  ...)
		end)
		...
		hook( ContourExt, '_upd_color', function( ... )
			...
			for i = 1, 4 do
				if not minionClr
					and (me:Stat(i,'minion')==self._unit
							or me:Stat(i,'minion2')==self._unit)
				then
...
--------------
	...
	if not inGame then
		hook( NewHeistsGui, 'init', function( ... )
			Run('init', ... )
			Poco._req('updates.lua')
		end)
